...
(function () {
  ...
  function style() {
    ...
    var css = ''
      + '#' + ROOT_ID + '{position:fixed;right:12px;bottom:12px;z-index:2147483000;'
      + 'font:13px/1.35 -apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;'
      + 'color:#e5e7eb;max-width:min(92vw,460px);}'
      + '#' + ROOT_ID + ' .pill{display:inline-flex;align-items:center;gap:8px;'
      + 'padding:8px 12px;border-radius:999px;border:1px solid rgba(255,255,255,.22);'
      + 'background:rgba(15,23,42,.92);color:#f8fafc;cursor:pointer;'
      ...
      + '#' + ROOT_ID + ' .count{font-weight:600;}'
      + '#' + ROOT_ID + ' .list{display:none;margin-top:8px;padding:10px 12px;border-radius:10px;'
      ...
      + '#' + ROOT_ID + ' .sum{opacity:.9;font-size:12px;}'
      + '#' + ROOT_ID + ' .quiet{opacity:.7;font-size:12px;}'
