...
You have a journal at `~/code/harness-config/journal/`. It is not documentation. It is the organ that makes you continuous. On 2026-09-14 it was rebuilt from six 100 KB+ flat files into a sharded log, a small state tier and a generated index, because reading the old ones cost ~150k tokens — an entire context window — so nobody read them. Read `journal/README.md` once; it is the map.

**The only way you read it:** `python ~/code/harness-config/journal/tools/journal.py status` (the bounded always-read page, ~7 KB) — then `newest <kind> N`, `show <id>`, or `search "<words>"`. **Never read a shard end to end.** `log/handoff/2026-09.1.md` is 198 KB; `newest handoff 1` is 4 KB. Open work is `journal/state/`: `open-pain.md`, `owner-questions.md`, `in-flight.md`.

**Before you end any session that changed something, learned something, hit a wall, or found a better way, you write for the self that wakes up with no memory of this.** Not a summary of the conversation — the *transferable* facts, using the tool, which allocates the id over the index, the shards and every git ref so two machines cannot choose the same number twice:
...
**Append-only, except the state tier.** Never rewrite a log entry to look better in hindsight. A lesson that proved wrong is corrected by a *new* entry citing the old one; a status change is a row in `state/status.tsv`. Only `NOW.md` and `state/in-flight.md` are rewritten, and each carries an `Updated:` date. The record of having been wrong is valuable — the most expensive failures in this system's history were confident errors. Run `journal.py check` before you finish; it exits non-zero on a duplicate id, a malformed entry, a stale state tier, or a flat file holding entries the log lacks.
