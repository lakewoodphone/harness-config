...
---
<!-- e:lessons|L213|2026-09-14|SECRATARY|open -->
**L213 · setsid nohup does not move a process out of its cgroup, and a login can reap what it started**

**2026-09-14 · secratary. Three phone outages in one night, one cause.**

The gate was started with `setsid nohup … &` — the idiom this repo uses everywhere for a long-lived
process, precisely so it survives the shell that started it. It does survive the *shell*; it does
**not** leave the **cgroup**. Measured:

```
/proc/<gate pid>/cgroup -> 0::/user.slice/user-1000.slice/session-25033.scope
...
The engine and the redirector were in the same position, in two other session scopes. When a session
ends, logind waits for its processes and removes the scope, taking them with it.

**Why it was so hard to see.** A signal leaves nothing in the process's own log: there were **zero
tracebacks** in a 2.5 MB gate log, no OOM line in `dmesg`/journal, 17.9 GB of RAM free, a fresh
process at 24 MB RSS, and no `kill` in any script on the host. The failure also correlated with
something irrelevant — every death was noticed when a browser page failed to load — so the first
three explanations were about the browser, the request load and the client cache. The instrument that
ended it was `/proc/<pid>/cgroup` plus the logind lifecycle lines, not more log reading.

**The rule.** A long-lived process on a host with systemd belongs to a **unit**, not to a shell:
`setsid` is for surviving a terminal, a unit is for surviving a session. Check the cgroup of anything
you intend to leave running — `cat /proc/<pid>/cgroup` — and if it says `session-*.scope`, it is on a
timer you did not set. And when a background process dies with no trace anywhere, suspect a **reaper**
before a bug: look at what owns its cgroup, not at what it was doing.

---
<!-- e:lessons|L214|2026-09-14|SECRATARY|open -->
**L214 · A UI complaint about size is usually a complaint about count**

**2026-09-14 · the owner: "the icons on the bottom of the messaging once the cost pill is there turn
into 2 layers cuz there isn't enough space … maybe make all the icons smaller".**

Measured at 393x852 on a session with usage, before touching anything: the composer occupied **232px
of 852** — 27% of the screen. Its input card wraps, so the controls stacked as `tools` (Commands,
Add attachment, Access mode, cost pill) at y=678 and `trailing` (model, context meter, Send) at
y=734, with the shipped stats strip on a third band at y=788.

"Make the icons smaller" was the wrong lever and would have made the phone worse: the 44px box is
the **thumb target**, and this same stylesheet exists partly to enforce that minimum. The space came
from **count**. Two controls were redundant on this device and removing them made the row fit on one
line:

- **access mode**, whose label there reads "Full access, current: Full access" — the host pins
 `permission.defaultPreset: danger-full-access`, so it could not change anything;
- **the stats strip** (turns/steps, tokens, cache hit), which restates the cost pill, the trajectory
 view and the context meter.

After: controls on one row (y≈470 and 472, i.e. the same line, against 678/734 before), no control
under 44px, and the access and stats controls collapsed to 0x0.

**The rule.** When a phone UI is described as too big, too crowded or "two layers", count the
controls before resizing them, and ask which of them can change anything on *this* device. A control
that cannot change state is not a small screen problem, it is clutter, and hiding it is what buys
the space. Shrinking a target trades a real cost (a thumbs-miss) for a cosmetic one.
