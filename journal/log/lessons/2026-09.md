...
---
<!-- e:lessons|L214|2026-09-14|SECRATARY|open -->
**L214 · The composer's two layers were flex-wrap, not width — composing the real DOM beat reading the CSS**

## What I learned

**A mobile layout bug that "came back" had never been fixed.** The journal and the 2026-09-14
commit `9eeea6e` record the composer as fixed ("the composer fits on one line"). The owner
photographed it still doubled on 2026-09-14 15:06. The prior change removed the Access-mode
control and the stats strip, which *reduced how often* the row overflowed; it did not remove
the overflow. Measured now, the row (`uV2eYG_row`) ships `flex-wrap: wrap` with
`justify-content: space-between` and the trailing group carrying `margin-left: auto`, so the
moment `tools + trailing + gap` exceeds the card's inner width the RIGHT group drops to a
second line while the left group keeps the first. That is the doubling, and it is a wrap.

**The trigger is content, not viewport width.** At 393 px the row is 351 px and the shipped
items need 232 px, so a fresh session looks perfect. It breaks when the session has usage:

| width | contents | row height | control bands |
|---|---|---|---|
| 393 | cost pill | 52 | 2 |
| 393 | pill + context meter | **104** | **3** |
| 393 | pill + meter + stop (active turn) | **104** | **3** |
| 430 | cost pill | **104** | **3** |

The cost pill comes from `plugin-cost` (registered in `conversation.input.left`, 20 px tall,
left group) and the context meter from `ui-conversation` (`JObwrW_trigger`, 28x28, trailing
group). A wider viewport is not safer than a narrow one — that is why this survived testing
at one width in one session state. **A layout claim measured in one session state is not a
layout claim.** Test the states the user actually reaches: empty session, loaded session,
active turn.

**The fix is one property, and it costs nothing at any measured width:**
`[class*="composer"] [class*="_row"] { flex-wrap: nowrap; }` plus `min-width: 0` /
`overflow: hidden` on the two groups so a long content can truncate instead of pushing the
send button off the card. Verified after the change, same probe, real served layer:
row 52 px and 2 bands at **320/358/375/393/430** with pill, pill+meter, and
pill+meter+stop, worst-case slack 8 px. The controls keep their 44 px boxes.

**How to measure a phone layout without a phone and without a browser driver.** Playwright's
browsers were cached but no driver was installed, so I drove Chrome headless over the
DevTools Protocol with the standard library only (~90 lines, `/tmp/mob/cdp.py` recipe):
a raw WebSocket client over `socket` (handshake, masked text frames, ping/pong), then
`Emulation.setDeviceMetricsOverride` (the only way to change the viewport —
`live.py`-style page-side resizing does not work), `Page.navigate`, and
`Runtime.evaluate` returning `{returnByValue: true}` JSON. It gave exact bounding boxes per
control, which is what distinguished a wrap from a stray element. `--dump-dom` also works for
markup and is the fastest way to answer "is the element there": the Access-mode control is
present in the DOM and `display: none` (`w: 0, h: 0`) — presence is not visibility, measure
the box.

**Headless limits worth knowing.** A synthetic click cannot start a turn in this app: the
composer input is a `contenteditable` div (not a `<textarea>`), `.value` does nothing, and
`element.focus()` does not take ownership in a headless target, so the send button stays
disabled. Compose the state by hand (`build()` in the probe inserts the real markup and
measures) rather than trying to drive the app end to end.

## Evidence

Live engine on `secratary` (`phone-gate` :3086 -> engine :3089), 2026-09-14 14:5x-15:2x UTC.
Probes in `/tmp/mob/`; the owner's own screenshot at 15:06 is the before state and matches
the `pill + meter` row of the table (104 px, two lines, send beside the field). Change:
`harness-config/assets/mobile.css`, the block "the composer row must not wrap".

---
<!-- e:lessons|L215|2026-09-14|SECRATARY|open -->
**L215 · A layout check must group controls by vertical centre, not by top edge**

## The mistake I made inside the fix for P66

Writing `scripts/phone-layout-check.py` right after fixing the composer, I asserted "all controls
on one band" by comparing each control's `getBoundingClientRect().y`. It failed on a **correct**
layout: the cost pill is 20 px tall and vertically centred in a 52 px row, so it reports
`y=472` while the 44 px send button reports `y=470`. Two bands for one row — the check accused
the fix it had just verified by hand.

**The rule:** a row is a set of controls sharing a *vertical centre*. Group on
`y + height/2` with a tolerance (12 px covers this app's 20/28/44 px controls and is far below
the 52 px row pitch), never on the top edge. A check whose pass condition does not match the
thing it measures is worse than no check: it produces a confident, wrong verdict — the same
failure class as L1/L2.

**Corollary worth keeping:** run a new check against the state you just fixed *and* against the
state before the fix. Against the pre-fix layer this check fails 13 of 15 cases (row 104 px,
two bands, composer 45% of the screen at 320 px); against the fixed layer it passes 15 of 15.
Without that control the "PASS" line would have meant nothing.

**Also learned:** the app really does render its own cost pill in some sessions — the first
production run of the check showed `$0.0049` twice (the app's, plus the one I composed), and
the measure function now records `preexisting.pills` so a future session can tell the two apart
instead of wondering why a fresh browser has a cost pill.

---
<!-- e:lessons|L217|2026-09-14|SECRATARY|open -->
**L217 · A question card on a phone must own the viewport: vh is a lie on iOS, and a tap test is the only honest check**
...
---
<!-- e:lessons|L218|2026-09-14|imported|open -->
**L218 · 2026-09-14 · A shared branch was rewritten under me; `push` printed success and nothing landed**
...
---
<!-- e:lessons|L219|2026-09-14|imported|open -->
**L219 · 2026-09-14 · A test whose harness is broken is worse than no test: it reports the broken harness as evidence. Check that the suite runs at all before trusting that it passes**
...
---
<!-- e:lessons|L220|2026-09-14|imported|open -->
**L220 · 2026-09-14 · One key cannot be both a flag and a count, and `Number(null)` is 0**
...
---
<!-- e:lessons|L221|2026-09-14|imported|open -->
**L221 · 2026-09-14 · An "unverified" tag is not a neutral result when it suppresses something that is true**
...
---
<!-- e:lessons|L222|2026-09-14|imported|open -->
**L222 · 2026-09-14 · Assert on the artefact that ships, not on the call that was supposed to produce it**
...
---
<!-- e:lessons|L223|2026-09-14|imported|open -->
**L223 · 2026-09-14 · A `git worktree` is not a copy of the working tree — untracked files stay behind, so it is the wrong control for environment-sensitive behaviour**
