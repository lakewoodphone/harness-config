...
---
<!-- e:decisions|D85|2026-09-14|ZABZ-TECH|open -->
**D85 · 2026-09-14 · A health surface reports what the data means, not what the enum says.**

**D (2026-09-14) - A health surface reports what the data means, not what the enum says.**

Settled while fixing a false alarm in `ha_truth.py` (see L230). Three rules, each paid for:

1. **Absent-by-intent is not absent-by-failure.** A config entry with `source: "ignore"` is a
 dismissed discovery; `not_loaded` is its correct, permanent state. It is excluded from
 degradation and reported separately as `DISMISSED_DISCOVERIES` at `low` severity -- visible,
 not alarming. Silently filtering it was the alternative and is worse: the estate is full of
 half-retired generations, and the next reader would rediscover it and call it broken again.
2. **A fault carries its duration.** `SECURITY_SENSORS_UNAVAILABLE` now says "the longest has
 been unavailable for 14.5 h". "Blind" and "blind for fourteen hours" were the same output and
 only one of them is urgent; that gap is exactly what P17 cost.
3. **A measurement the surface could always have taken belongs on the surface.** Batteries were
 never reported even though every battery entity was already in the payload being read. Two
 door contacts were silent for 14 hours with a last reading of 8% and 21%, and nothing said so.

Read from the **entity surface**, not from the radio, so the same finding works for
Zigbee2MQTT, ZHA, Z-Wave and ESPHome with no extra credential. `sensor.` domain only: a
`binary_sensor` with `device_class: battery` is a low-battery flag, and reading `off` as a level
invents a number.

**Recorded, deliberately not done:** bounding the retry storm in the camera snapshot scripts.
They are called by the security automations and retry internally, producing ~500 failed logins
per hour against the NVR. That is very likely keeping the NVR's admin account locked out, which
would mean the stored password is correct and the camera path could recover without the owner.
It is left alone this session because it is a change to the live security chain and the owner
question (queue #23) settles the same fact more cheaply.

---
<!-- e:decisions|D86|2026-09-14|ZABZ-TECH|open -->
**D86 · 2026-09-14 · A breached password is rotated, not exempted.**

**D (2026-09-14) - A breached password is rotated, not exempted.**

The `nut` app refuses to start when `users[0].password` appears in HaveIBeenPwned (L233). The
tempting fix is to weaken or bypass the check. Not done: the check is correct, the password was
`Hello@1234`, and that same password was sitting in all six Dahua camera entries.

Rotated to a generated 40-character value, propagated to the HA entry through the integration's
own reconfigure flow, and verified end to end. The add-on's policy stays intact.

**Corollary recorded for the owner, not decided here:** `Hello@1234` appearing in both the UPS
add-on and the camera config entries is a reuse pattern, and the NVR is now refusing it. If the
NVR password turns out to be unchanged, the likely cause is an account lockout from HA's own
retry storm (see queue #23), not a wrong secret.
...
---
<!-- e:decisions|D88|2026-09-14|ZABZ-TECH|open -->
**D88 · 2026-09-14 · Order the laptopscreen.com 40-pin touch panel for the 82XM00LMUS job, and hold the  quote.**

DECIDED 2026-09-14, by me, under the standing split: development and sourcing calls are mine, the
owner's are money, customers, legal, and irreversible things.

THE CALL: order laptopscreen.com item 9597874 - "Lenovo IDEAPAD SLIM 3 15ABR8 MODEL 82XM" LCD,
IPS On-Cell Touch, 15.6" FHD 1920x1080, matte, 40 pin narrow eDP, no brackets, New Grade A+,
lifetime warranty, with double-sided tape. $76.99 return-customer / $86.99 new. 1000+ in stock.
Ships same day from Newark, DE. Read live and verified by me 2026-09-14.

WHY, over the alternatives
 - It is LPT's EXISTING laptop-screen supplier (Margulies IdeaPad 15IJL7, Dell Latitude 3540,
 Dell Inspiron 15 3535, HP 15-DY5131WM all sourced there), so no new account or vendor risk.
 - Same price band as the fastest eBay option but a real lifetime warranty and 1000+ in stock.
 - Item number is explicitly the 40-pin TOUCH panel, which is the failure mode on this job.
REJECTED
 - Amazon B0DDRRMRVL SCREENARAMA $69.95 - correct part and cheapest, but Amazon-stated delivery
 Sat 2026-09-19, and this customer is a college student already 4 days into a broken promise.
 $17 is not worth 3 extra days here.
 - eBay 197794603503 FIXARAMA $75.98, FRU 5D10W46422, Wed-Thu - genuinely competitive; kept as
 the fallback if laptopscreen.com will not ship today.
 - Every sub-$57 listing for "15ABR8 Type 82XM" - the 30-pin NON-TOUCH panel. Wrong part.
 - Used complete assembly at $55.99, no returns - not for a customer machine.

HOLD THE QUOTE at $175 for the screen. At a $77-87 part that is a sound margin and the customer has
already been told the number.

STILL UNRESOLVED AND NOT MINE TO GUESS
 - The Lenovo FRU for this touch panel. The IdeaPad Slim 3 HMM omits FRU numbers entirely and
 routes everything to a JS-only parts lookup that needs the REAL serial, which the label photo
 could not give me. 5D10W46422 is supplier-claimed and corrobed by two independent sources
 only. Not needed to buy the part from laptopscreen.com.

EVIDENCE
lpt-hub docs/customer-operations/sync-records/2026-09-14-lenovo-ideapad-slim3-15abr8-screen.json
lpt-hub research/hardware/laptop-repair/2026-09-14-lenovo-82XM00LMUS-lcd-sourcing.md
...
---
<!-- e:decisions|D90|2026-09-14|imported|open -->
**D90 · 2026-09-14 · The filter gets its own website on an LPT subdomain, signing in with LPT identity through a server-side BFF — and a missing configuration value must close a door, never open one.**
...
---
<!-- e:decisions|D92|2026-09-14|ZABZ-TECH|open -->
**D92 · 2026-09-14 · Case system: case file is the record, sync record is a derived mirror, retrieval is index-first (lptcase).**

DECIDED 2026-09-14 (owner: "audit how you write, pull up and find case files... way better more robust tools").
1. The CASE FILE is the record of a case: docs/customer-operations/cases/<device>-<customer>-<phone>-<date>.md,
 with a machine-readable YAML front-matter block carrying current state and an append-only dated activity log.
 The sync-records/*.json is a DERIVED MIRROR to production; when they disagree the case file wins.
 (Rejected the audit's alternative of making the JSON authoritative: the prose log holds the evidence.)
2. Retrieval is index-first: lpt-hub/tools/case-index/lptcase.py (stdlib-only, single file) builds
 index/cases.json + cases.md + cases.db from the corpus; every query answers in <1 s.
3. New case files are created with `lptcase.py new <slug>` so the header exists from intake - the corpus
 reached 179 files with ~0% front-matter and 120 distinct status literals.
4. The owner's misspellings and any second phone number are recorded as ALIASES in the case file, so a
 wrong spelling or the other number still finds the case (proved: 732-678-6417 finds the Abramczyk case).
5. lpt-hub/AGENTS.md §0 now forbids tree-grepping for a case and points at the tool; §5 no longer sends
 the research agent to index/master-index.json for customer questions (it covers 0 cases, 136 days stale).

---
<!-- e:decisions|D93|2026-09-14|ZABZ-TECH|open -->
**D93 · 2026-09-14 · Weitman laptop case: case artifacts, an authoritative null price in lptcase, and no intake record for a sale.**

Context: case 14in-laptop-weitman-8483336341-2026-09-14 (customer Weitman / production 1624 wants to buy
a 14in non-touch entry laptop). All four are dev decisions, made and recorded without asking the owner.

D1. CASE ARTIFACTS. Created with the scaffolder, not by hand:
 docs/customer-operations/cases/14in-laptop-weitman-8483336341-2026-09-14.md
 docs/customer-operations/sync-records/2026-09-14-weitman-laptop-sourcing.json
 Naming follows CASE-FILE-STANDARD §1 (<device>-<customer>-<phone>-<opened>), case_id == filename stem,
 front-matter carries current state, dated ### sections are the append-only log. Why: the case must be
 findable by phone through tools/case-index/lptcase.py, never by tree-grep (AGENTS §0).

D2. lptcase.py: AN EXPLICIT `price: null` IS NOW AUTHORITATIVE. The extractor built its price from the
 front-matter, the header region and any Scope/Pricing/Quote section, then took the max. With
 `price: null` the tool still reported `quote $40`, harvested from a header sentence describing an
 unrelated June work order. CASE-FILE-STANDARD §2 says the front-matter overrides the body, so a
 declared-null price now suppresses every body-derived price. Measured the blast radius before
 changing it: of 180 case files, 177 have no front-matter; of the 2 with a null/empty price exactly
 1 (this new one) had a body-derived price, so nothing existing changes. Two tests added
 (test_front_matter_null_price_suppresses_body_prices, test_front_matter_real_price_still_wins_over_prose);
 suite 59 passed.

D3. NO SECRETARY INTAKE RECORD FOR THIS CASE - DELIBERATE. POST /api/v1/shop/customer-intake always
 creates a task titled "Repair: <device> - <name>" with status CHECKED_IN. This intake is a laptop
 SALE, not a repair with a device on the bench, so calling it would plant a misleading repair task in
 the company's task system. Not called. The gap (no contact/task for this phone) is written into the
 case file and the sync record instead. The real fix is an intake endpoint that takes a case type;
 until then, do not use it for sales.

D4. THE FORM-SOURCING FLOW APPLIES EVEN THOUGH HE DID NOT COME THROUGH THE FORM. The customer asked by
 SMS, so scripts/pull-google-forms.py and process-computer-form-leads.py do not apply. Everything else
 in docs/customer-operations/computer-form-sourcing-workflow.md does: mandatory multi-source coverage
 (amazon, ebay, bestbuy, newegg, dell_refurbished, research_dispatcher), the durable ledger
 (scripts/laptop_sourcing.py), the customer-total formula source x 1.36 x 1.06625, and the ban on ever
 disclosing marketplace, seller, source price, item number or link to a customer.

---
<!-- e:decisions|D94|2026-09-14|ZABZ-TECH|open -->
**D94 · 2026-09-14 · Owner's repair pricing rule, stated directly: (cost + shipping) x 1.4 + 65 - now canonical at lpt-hub/docs/customer-operations/PRICING-RULE.md.**

The owner gave the shop's repair pricing rule directly, 2026-09-14:

 customer price = (part cost + shipping) * 1.4 + 65 then round to a sensible number

He said it while setting one job's price: "it's 180 then, because cost plus shipping x 1.4 plus 65".

WHY THIS IS RECORDED AND NOT JUST APPLIED ONCE. Until today this rule existed only inside one case
file (hp-15-fd0095wm-shimmy-weichbrud-2026-09-03: "cost x 1.4 + $65") and one line of the Weinberg
employment agreement ("Parts: 1.4x cost markup"). A future session quoting a repair had to find it
by luck. It is now canonical at lpt-hub docs/customer-operations/PRICING-RULE.md.

DO NOT CONFUSE IT WITH THE RESALE MARKUP. Whole-laptop resale/sourcing is a DIFFERENT, smaller
multiplier: 1.30-1.36x (owner's own 2026-08-05 correction in the Tchiprout MacBook case, "standard
markup = ~30%, not 1.36x"). Repair = (cost+shipping)*1.4+65. The two live in the same repo and
getting them crossed misprices every job in one direction or the other. Both are in PRICING-RULE.md
with a table.

TWO THINGS THAT MATTER IN THE FORMULA
1. SHIPPING IS REAL, NOT AN ALLOWANCE. laptopscreen.com ships one screen USPS Priority for about
 $8.88 to NY/NJ and is free only at 10+ screens. Assuming "free shipping" is the default case
 understates the price. Measured off the supplier's own shipping page, not guessed.
2. THE SUPPLIER IS PART OF THE PRICE. Same formula gave $165 on a $69.95 Amazon panel and $185 on a
 $76.99 laptopscreen.com panel for the same repair. Never quote before the LANDED cost is real.

WORKED, on the 2026-09-14 Lenovo 82XM00LMUS screen:
 (76.99 + 8.88) * 1.4 + 65 = 85.87 * 1.4 + 65 = 120.22 + 65 = 185.22 -> $185

The owner's own stated number that day was $180 - which corresponds to a ~$82 landed cost, i.e. a
different cost base than the order recommended. Recorded as a known $5 gap rather than silently
overwritten, and the one word from him that would settle it is noted in the case file.
...
---
<!-- e:decisions|D97|2026-09-14|ZABZ-TECH|open -->
**D97 · 2026-09-14 · Weitman: close the old jobs, correct production rather than leave it wrong, and make a real front-matter price authoritative.**

Context: the Weitman laptop case, after the owner answered on 2026-09-14. All dev/ops decisions; none
were routed to him.

D1. THE TWO OLD JOBS ARE CLOSED, NOT CARRIED. HP back cover: COMPLETED + PAID (65). Lenovo USB port:
UNFIXABLE because the correct jack could not be sourced - no charge, nothing owed, NO CREDIT. I had
recommended turning the 60 into a credit off the laptop; the owner did not choose that, so there is no
credit. Do not invent one, and do not re-raise the port as an open repair.

D2. PRODUCTION WAS CORRECTED RATHER THAN LEFT WRONG. WO26082712635 -> COMPLETED/CAPTURED,
WO26082712634 -> UNFIXABLE with its total zeroed, plus customer note 2707. Reasoning: a shop record
that calls an unfixable repair READY_FOR_PICKUP with a PENDING balance is not a neutral error - it
makes staff chase money that is not owed and it hides a broken promise. The write went through
app.services.website_db.update_order (validated field set, confidence 0.98, OrderStatusHistory row)
and was read back. Two production facts future sessions must not rediscover: the PaymentStatus enum is
CAPTURED|PENDING, so "PAID" is REJECTED; and orders.statusId is null on every row, so the text status
column is the live field.

D3. THE FRONT-MATTER PRICE IS NOW THE ONLY QUOTE. lptcase took the MAX of the front-matter, the header
region and any Scope/Pricing section. The HP case declares `price: 65` and the tool reported 135 - the
combined two-laptop quote mentioned in its own header. An explicit real front-matter price now
suppresses body-derived figures exactly as `price: null` already did. Blast radius measured first: of
180 case files only 5 carry front-matter and only this pair changes. Three tests; suite 62 passed.

D4. THE CUSTOMER'S GENDER IS NOW IN BOTH RECORDS. Production said "mrs whiteman" from the start and
the first draft of the case still used he/him throughout. Both the case file and the sync record now
carry the gender note, so the next session does not re-guess, and the case files' `aliases` carry
every spelling anyone has used (Weitman / Whiteman / mrs whiteman / whitman).
...
---
<!-- e:decisions|D100|2026-09-14|ZABZ-TECH|open -->
**D100 · 2026-09-14 · The 121 dollar buy ceiling has no supply: verified iPhone 11 lots are 135.80 and 175 per unit, and the earlier 112.50 data point could not be reproduced.**

## D100 · 2026-09-14 · The $121 buy ceiling has no supply behind it: every verifiable iPhone 11 lot is $135.80-175/unit, so the price or the ceiling has to move.

Went looking for the live listings that would satisfy the ceiling set earlier today, and could not find one. Every
lot I could open was measured directly, not inferred:

| Listing | Lot | Per unit |
|---|---|---|
| "Lot of 5 Apple iPhone 11 A2111 64GB Unlocked Clean IMEI: Fair" — Sage Sustainable Electronics (ebay.com/itm/257708163947) | 5 | **$135.80** ($679, free UPS; grade "Fair"; seller states some units may carry third-party replacement parts) |
| "Apple iPhone 11 64GB Fully Unlocked Excellent [LOT OF 10]" — CREO Cellular (ebay.com/itm/177336778913) | 10 | **$175.00** ($1,749.99, battery 90%+) |
| Graded singles: Back Market Good, eBay singles, Cellpex | 1 | $137-158 |

**Retraction:** the "$112.50/unit on a 2-unit eBay lot" figure used earlier today **could not be reproduced** — the
item it came from now reads as a 5-unit lot at $679. Either the listing changed or the attribution was wrong.
It is now marked unverified in the pipeline doc, and the ceilings derived from it should not be trusted as
reachable.

**The consequence, as arithmetic rather than opinion.** List − $100 − $37.94 of case, cable, box and prep = the
buy ceiling:

| List price | Ceiling | Supply today |
|---|---|---|
| $259 | $121 | **none verified** |
| $279 | $141 | the $135.80 5-lot qualifies; singles at $140-141 come into range |
| $289 | $151 | singles to $151 and Back Market at $144 **with a 1-year warranty** qualify |

**Raised to the owner as a price question, because price is his.** My recommendation is **$279**: it keeps the
≥$100 floor exactly ($100.06 at the $141 ceiling, $105.26 on the $135.80 lot as measured today) and converts a
programme with no supply into one that can buy this week. $289 buys warranty-covered stock instead of bare lots,
which is worth something against an 8-month warranty obligation.

**Not decided by me.** Until the owner rules, the standing answer on any listing above $121 is: do not buy.
Nothing has been ordered or paid.

---
<!-- e:decisions|D101|2026-09-14|ZABZ-TECH|open -->
**D101 · 2026-09-14 · List price 299 dollars and buy ceiling 153: the price move that turned a supply-less programme into one that can buy a verified 5-lot today at 135.80 a unit.**

## D101 · 2026-09-14 · List price $299 and buy ceiling $153 — the price move that turned a programme with no supply into one that can buy today.

Owner ruling: *"we don't use warranties from marketplaces so much usually anyways, it can be up to 300 for us to
charge. remember it has to cover case and phone and anything else and still make 100 profit."*

**Set: list $299, buy ceiling $153.**

The ceiling is computed on the **conservative** all-in non-device cost, not the optimistic one, because the owner's
constraint is that the price must cover everything and *still* clear $100: 299 − 100 − 46 = **153**. The non-device
cost is case $21.95 (Amazon SUPCASE UB Pro) + pad print $1.45 + plate amortised over 50 ($3.00-4.80) + cable $5.66
bulk (~$10 at retail for the first 20) + branded tie $0.53 + box and EVA insert $3.35-5.10 + a $2.00
provisioning/QC allowance = **$38 low, $46 conservative**. If cases and boxes land low the ceiling could rise to
$161, but the rule stays at $153 so the floor is never at risk.

**Effect on supply, which was the entire problem:** at $153 the verified market is now buyable rather than
theoretical.

| Listing | Lot | Per unit | Profit at $299 |
|---|---|---|---|
| eBay "Lot of 5 iPhone 11 A2111 64GB Unlocked Clean IMEI: Fair" — Sage Sustainable Electronics | 5 | **$135.80** | **$117.20** |
| Cellpex published, iPhone 11 used | 10 (MOQ) | $137.00 | $116.00 |
| Back Market, Good, 1-year warranty | 1 | $144.00 | $109.00 |
| eBay graded singles | 1 | $140-149 | $104-113 |

**First buy card is now real** (in the pipeline doc): the 5-lot at $679 = $135.80/unit, plus one SUPCASE sample,
50 EVA inserts and 20 retail MFi cables — **~$943 total, all for the owner to place.** The lot is grade "Fair" with
third-party replacement parts possible, so it is QC-heavy: activation lock, IMEI, battery and screen checked on
arrival, and failures become parts stock.

**Retraction carried into the doc:** the "$112.50/unit 2-unit eBay lot" figure was withdrawn as unverifiable, and
the table and ceiling derived from it were replaced (D100). The pipeline doc's buy-ceiling table now carries only
numbers that were opened and read today.
...
---
<!-- e:decisions|D103|2026-09-14|ZABZ-TECH|open -->
**D103 · 2026-09-14 · 2026-09-14: the same-day Microsoft and Verizon account-security changes are the owner's own - verified benign, not a takeover.**

Owner answered 2026-09-14, on being shown the two same-day account-security changes: **"That's us — ignore both."**
- Microsoft security-info replacement on `ki**i@outlook.com` -> `perelplutchok@gmail.com`, effective 2026-09-30: LEGITIMATE, family. Do not raise again, do not treat as a takeover.
- Verizon account XXXXXX-674-0001 password + secret question change and Face ID enablement, 2026-09-14 11:02 ET: LEGITIMATE.

Why this is recorded: two account-security changes on two accounts within 90 minutes is exactly the
pattern of a real intrusion, and it is the shape a future session should still check for on every
sweep. It was benign THIS time. The check stays; the alarm for this instance is closed.
