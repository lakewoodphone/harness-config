...
---
<!-- e:decisions|D71|2026-09-14|SECRATARY|open -->
**D71 · 2026-09-14 · systemd owns the phone's two long-lived processes; the phone workspace is ~/phone, and _scratch stays for history.**

**Decided and applied 2026-09-14 by Zabz.** Development decisions, taken without asking.

**1. systemd owns the engine and the gate.** `phone-engine.service` and `phone-gate.service`
(`User=zabz`, `Restart=always`, `RestartSec` 2-3 s, `LimitNOFILE=8192` on the gate — a relay holds two
descriptors per streamed connection and the default soft limit is 1024). They live in
`/system.slice/`, which no login session can reap (the lesson above), and the recovery time drops
from up-to-a-watchdog-interval to two seconds.

**2. Exactly one supervisor per process.** `serve-phone.sh` now *defers*: start/stop/status go
through `systemctl` when the unit exists, and it never restarts a healthy unit. The cron entry stays
for Serve, the redirector, the plugin installs and the workspace, all of which are idempotent checks
rather than process ownership. The reason is measured, not theoretical: the API on this same host has
two supervisors and they fight (see the pain entry above).

**3. The phone workspace is `/home/zabz/phone`, titled "Phone".** Deliberately **not** a git
checkout: a phone conversation can run shell commands in its working directory, and a repository of
the company's code must not be reachable by accident from a pocket. `scripts/ensure-phone-workspace.py`
keeps it registered, because the workspace list is host state in `~/.dsh/storages/workspace.json`
that no git checkout can restore and whose absence shows up as "choose a workspace" on every new
conversation.

**4. `_scratch` is kept, not renamed.** Nine sessions record it as their cwd, and DSH refuses to
attach a session whose stored directory does not resolve — renaming it would make the owner's own
history unopenable. One extra entry in a list is the cheaper cost. Verified from the phone page: a
new session now lands in `/home/zabz/phone`.

**What would reverse this:** a second always-on host serving the phone (then the units move with it),
or a harness that ships mobile layout (then the injected layer and the plugins become redundant).
