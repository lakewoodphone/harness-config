...
---
<!-- e:wins|W50|2026-09-14|imported|open -->
**W50 · 2026-09-14 · Four self-inflicted defects found by measuring the artefact, and one of them was deleting the biggest conversation in the archive.**
...
---
<!-- e:wins|W51|2026-09-14|imported|open -->
**W51 · 2026-09-14 · A year of the owner's business conversation is now searchable, and nothing was.**
...
---
<!-- e:wins|W52|2026-09-14|imported|open -->
**W52 · 2026-09-14 · The filter's first full state audit found a live-adjacent security defect and a product that is far more built than any document said — and both were proved rather than asserted.**
...
---
<!-- e:wins|W53|2026-09-14|ZABZ-TECH|open -->
**W53 · 2026-09-14 · lptcase: 179 cases indexed in 0.46s, fuzzy/phone/body search in ~40ms vs 59s cold ripgrep.**

MEASURED 2026-09-14 (lpt-hub, 179 case files):
- `lptcase.py index` builds index/cases.json + cases.md + cases.db in 0.46 s for the whole corpus.
- Warm queries answer in 31-44 ms: `find abramzyc` -> Abramczyk (name-fuzzy 0.82), `find 8482857640` ->
 Abramczyk (phone), `find "sd card"` -> the SD cases, `find 7326786417` -> the Abramczyk case via the
 second phone number written in the case BODY. The same lookups by ripgrep took 59.4 s cold and missed
 the misspelled and second-number cases entirely.
- `show` returns state, money, work order and the newest sections in bounded output (never a 250-line dump).
- The broken junction at personal-secretary-mvp\lpt-hub was rebuilt with rmdir + mklink /J; repo-wide
 search through that path works again (179 cases visible where it previously returned 0).

---
<!-- e:wins|W54|2026-09-14|ZABZ-TECH|open -->
**W54 · 2026-09-14 · A retry that cannot succeed is not a retry, it is a lockout.**

**A retry that cannot succeed is not a retry, it is a lockout. The camera evidence path was dead for days because Home Assistant kept knocking.**

Measured 2026-09-14, on the live estate, after the owner confirmed his NVR password was the one
Home Assistant already held.

The chain, each link measured rather than assumed:

1. The Dahua NVR locks an admin account after a handful of failed logins, and further attempts
 keep it locked.
2. `script.snapshot_latest_with_retries` retried **3 times per camera**, and
 `script.snapshot_entry_cameras` sweeps **4 cameras**, and the whole thing ran on
 **`binary_sensor.video_doorbell_person`** -- 39 real visitors in 8 hours (25 of them in one
 hour). That is ~500 failed logins an hour against a shop with a front door.
3. So the NVR never saw the quiet window it needed to unlock, and a **correct** password looked
 like a wrong one from every angle I could test -- curl `--digest`, `--basic`, `--anyauth`, the
 local web UI, and the integration's own call. All 401/500.
4. The integration reported `loaded` and kept every camera entity `available` through all of it,
 which is why nothing in the entity surface ever said the evidence path was dead.

**What was built.** `scripts/ha_circuit_breaker` -- actually `config/scripts.yaml` ->
`snapshot_latest_with_retries`, plus `config/timeline_tools/snapshot_probe.sh`: the script now
**deletes the target file, attempts the snapshot, and then asks the disk whether a file
appeared**. Five consecutive snapshots that produce no file open
`timer.phoenix_snapshot_breaker` for 30 minutes, and while it is open the script stops before
touching any camera. A success resets the counter. Opening the breaker writes a logbook entry
that says, in words, *check the credentials, do not just retry*.

**Verified live, both paths** (not assumed -- driven through `script.turn_on` and read back from
the entity surface and the script trace): failures counted 1, 2, 3, 4; the breaker opened on the
fifth and reset the counter; runs six and seven stopped without reaching the camera. Then, after
cancelling the breaker, two failures followed by one success left the counter at **0**.

**Two things the testing taught that reading would not.**
- `camera.snapshot` **creates the parent directory**, so pointing the script at a nonexistent
 directory is not a failure -- my first "failing" test wrote a 46 KB JPEG and the breaker
 correctly declined to trip. A guaranteed-unwritable path is one whose parent is a *file*.
- The failure signal had to be the file, because the integration lies by omission: `loaded` +
 `available` while every call fails.

**Left alone deliberately:** the retry count for a *transient* blip is unchanged, and the
doorbell trigger is untouched -- those 39 visitors are the business, not a bug.
