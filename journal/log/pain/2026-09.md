...
---
<!-- e:pain|P86|2026-09-14|ZABZ-TECH|open -->
## P86 — lpt-hub exists twice, both copies are written to, and the authority's mirror lands on the copy nobody reads

RANK: high. It silently splits the record of the whole shop in two.

TWO lpt-hub CHECKOUTS EXIST AND BOTH GET WRITTEN TO.
 - C:\Users\ezabz\Code\lpt-hub on ZABZ-TECH (the desktop repo). On this machine
 personal-secretary-mvp/lpt-hub is a Windows JUNCTION to it, so there is exactly one
 copy here.
 - /home/zabz/personal-secretary-mvp/lpt-hub ON SECRATARY. That one is a real directory
 on a different machine, not a junction to the desktop.

EVIDENCE, 2026-09-14: POSTing to the secretary shop intake API created a sync record and
returned its path -
 /home/zabz/personal-secretary-mvp/lpt-hub/docs/customer-operations/sync-records/
 lenovo-ideapad-slim-3-15abr8-(mtm-82xm00lmus)-customer-(848)-480-5115-2026-09-14.json
That file is a mirror of a customer case whose case file lives only in the desktop repo.
Nobody reading the desktop repo will ever see it. The two also exchange work in the other
direction (a session on the desktop pushed case files the server does not have), so
neither copy is complete.

WHY IT HURTS. The case system's entire promise is "the case file is the record and the
sync record mirrors it". A mirror written to a different machine, under a different naming
scheme, that the linker on the desktop cannot see, is not a mirror - it is a second
source of truth that drifts. This is the same class of failure as P3 (reading the wrong
data and believing it) but in reverse: writing the right data somewhere nobody reads.

SECOND, COMPOUNDING. The API also names its record
"lenovo-ideapad-slim-3-15abr8-(mtm-82xm00lmus)-customer-(848)-480-5115-2026-09-14" -
device + MTM + a PHONE NUMBER as the customer slug + parentheses. It matches no case file
and violates CASE-FILE-STANDARD (which wants <device>-<customer>-<10-digit-phone>-<date>).
personal-secretary-mvp/app/services/shop_ops.py:205 builds it as
 f"{make}-{model}-{customer_name}-{date}" lowercased with spaces -> dashes
so every case created through the authority is born an orphan by construction.

WHAT WOULD FIX IT
1. One source of truth for lpt-hub. Either the server stops holding a clone and reaches
 the desktop's (or the GitHub remote) instead, or the server's copy IS the only one and
 the desktop junction points at it. Decide, then make the other path impossible to
 write to by accident.
2. shop_ops.customer_intake should emit the CASE-FILE-STANDARD name -
 <device-slug>-<customer-slug>-<phone-digits>-<date> - so the record links by filename
 with no heuristics. Then deploy it.

NOT DONE, and not done deliberately: reconciling the two clones means deciding which is
authoritative and possibly moving customer data between machines. That is an owner call on
where the record lives, and it is queued rather than guessed.

---
<!-- e:pain|P87|2026-09-14|ZABZ-TECH|open -->
## P87 — Device buy ceilings are set from prices that cannot be reproduced: a single unverified listing number became a hard purchasing rule within the hour
...
---
<!-- e:pain|P88|2026-09-14|ZABZ-TECH|open -->
## P88 — The photo-condition check was dead for want of one empty API key, and that is how a beaten-up laptop got offered to the owner

RANK: high. It is the reason a customer-facing sourcing decision was made on a seller's grade.

MEASURED 2026-09-14 on ZABZ-TECH. `.env` in personal-secretary-mvp has line 491 `MINIMAX_API_KEY=`
with a value LENGTH OF ZERO. Every other provider key is populated (OPENAI_API_KEY 164 chars,
ANTHROPIC 108, GOOGLE 39, GROQ 56, COHERE 40, TOGETHER 50, DEEPSEEK 35, PERPLEXITY 53, JINA 65,
FIRECRAWL 35).

CONSEQUENCE, and it is not theoretical. `scripts/analyze_image.py` returned:
 IMAGE_ANALYSIS_FAILED: missing MINIMAX_API_KEY in .env; cannot run MiniMax vision
so `laptop_sourcing.py record --analyze-images` could not analyse a single listing photo. The
photo-condition check is the step that stops a cosmetically beaten-up used laptop being offered to
the owner or sold to a customer. With it dead, the ledger fell back on listing GRADES - and a "Very
Good - Refurbished" eBay Dell Latitude 7400 with visible wear was presented to the owner, who opened
the photos and rejected it: "even the first one has pretty bad cosmetic damage, come on." I had even
written "photos not analysed" into the case file and presented the listing anyway.

FIXED AND VERIFIED (mvp commit 25032e92). `laptop_llm.minimax_vision` now builds a provider chain -
the configured primary first, then any configured fallback whose key is populated - so a missing key
on one provider no longer means no photo review at all. Fallbacks: OpenAI gpt-4o-mini, then Google
gemini-2.5-flash, both OpenAI-compatible. Failure still returns the IMAGE_ANALYSIS_FAILED marker so
the scorer treats a dead call as unusable rather than clean. Verified live by re-running the analysis
on the rejected listing's own six photos: provider=openai, imgs=8, a real assessment returned.

OPEN, low priority: decide whether to refill MINIMAX_API_KEY, or drop MiniMax from the vision config
entirely so the primary is a provider that actually works. The fallback makes this non-urgent, but the
config still names a dead primary.

---
<!-- e:pain|P89|2026-09-14|ZABZ-TECH|open -->
## P89 — Attention-feeding capture pipelines die silently: Dialpad voicemail capture stopped 2026-06-14 and call_log stopped 2026-08-12, so no session can tell whether a customer was answered

**The capture pipelines that feed attention died silently and nobody noticed for months.**
- `dialpad_ui_voicemail_row` (authority secretary.db): 3,036 rows, MIN 2026-04-24, MAX 2026-06-14.
 Stopped three months ago. Every voicemail since exists only as a Dialpad notification email in one
 Gmail inbox, unread.
- `call_log` (same db): 70 rows, MAX 2026-08-12. No outbound-call record for a month.
- `dialpad_ui_history_row` and `dialpad_ui_voicemail_row` both stale, which is why the question "did
 anyone call this customer back?" cannot be answered from our own data at all.

Why it hurts: it converts live customers into an unread email pile, and it removes the only evidence
that would let me tell an owner whether his shop is actually responding. Measured tonight: an owner
looking at his inbox cannot distinguish "shop handled it" from "customer is waiting".

What would fix it: a freshness assertion, not a bigger crawler. One monitor that reads MAX(captured_at)
for each attention-feeding table and raises when it is older than its expected cadence (voicemails
daily, calls hourly), so a dead harvester is a loud fact within a day instead of a silent one for a
quarter. Same shape as the fix for the Deep Infra / AliExpress duplicate-draft loops: a pipeline with
no freshness or dedupe assertion is a pipeline that is already broken, you just do not know it yet.

---
<!-- e:pain|P90|2026-09-14|ZABZ-TECH|open -->
## P90 — Our 1.45x laptop formula makes an entry laptop bad value, and the owner can see it

RAISED BY THE OWNER 2026-09-14: "470 for a bad laptop like this does not seem great", with the Amazon page
for the Lenovo IdeaPad 3i 14 (i3-1115G4, 8GB/256GB, FHD) open at 342.00.

HE IS RIGHT, AND THE ARITHMETIC IS THE WHOLE POINT. The shop's customer-facing total is
source x 1.36 x 1.06625 = source x 1.4501. On a 342 laptop that is 495.94 - a markup of 153.94 on a
machine the customer can buy herself on Amazon for 342. Nothing we add (sourcing, checking, setup,
support) is visibly worth 154 on a 342 box, and the customer can see the retail price of the exact
model. The percentage model works on a 1,000 laptop and fails on a 350 one: same 45%, but on cheap
machines the fee is either too small for the shop or too visible for the customer.

AND THE SOURCING CANNOT RESCUE IT. Genuinely new 14in non-touch with 8GB/256GB starts at 323 (seven
channels swept 2026-09-14) and the machines at that floor are dual-core 2021 silicon. Clean
refurbished costs the SAME as new (Certified 319.99 vs new 323.00), so the cheap-and-good used lane
does not exist. The only genuinely good machine found near the brief is the ASUS Vivobook 14
i5-1235U 10-core, 8GB/256GB, 1920x1080 at 445.49 - 645.92 through us. So the choice is: a weak laptop
at ~468-496, or a good one at ~646, or no laptop.

THE REAL QUESTION, AND IT IS THE OWNER'S BECAUSE IT IS PRICING: does the shop keep the 1.4501x on
laptops and only sell machines where the money buys something worth owning, or does it move the entry
tier to a flat sourcing+setup fee, or does it stop selling boxes at this tier and sell the service
instead (she buys at retail, we set it up)? The formula is a business rule he set; changing it is
money. Raised to him as one question with a recommendation rather than decided.

ALSO WORTH KNOWING FOR THE NEXT TIME: the 14in requirement is costing money. Entry-tier value in the
US sits at 15.6in - the same Amazon page shows 15.6in FHD machines with Office bundles at 339-420
against 14in at 342-445 for weaker specs. The owner's own words were "PROBABLY 14 inch", so the
constraint is soft and is worth confirming with the customer rather than assumed.
