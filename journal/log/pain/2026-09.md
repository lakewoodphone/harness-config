...
## P66 — The company API has two supervisors and one of them kills the other's process

**Found 2026-09-14 while chasing an unrelated death, on the authority.**

`secretary-api.service` is a systemd unit with `Restart=always` / `RestartSec=5` / `KillMode=control-group`,
**and** the user crontab also runs, every five minutes:

```
*/5 * * * * pgrep -f "uvicorn app.main" > /dev/null || ~/personal-secretary-mvp/scripts/secretary-startup.sh
```

That script's own first act is to kill what it finds:

```
EXISTING_PID=$(pgrep -f "uvicorn app.main" || true)
if [ -n "$EXISTING_PID" ]; then log "INFO" "Killing PID $EXISTING_PID..."; kill "$EXISTING_PID"; sleep 2
 if kill -0 "$EXISTING_PID"; then kill -9 "$EXISTING_PID"; fi
fi
```

So whenever the cron watchdog cannot see the service (or decides it is down), it **kills the process
systemd owns** and starts an unmanaged `nohup` copy; systemd sees its MainPID die and starts another;
the next cron pass repeats it. Measured: **4 `Stopping secretary-api.service` events in the 14:00
hour** — and 12 stops today, 43 on 2026-09-13 — with the app logging a graceful `INFO: Shutting down`
each time, i.e. killed by SIGTERM from systemd rather than crashing. `~/secretary-api.pid` still says
`93586`, a process from another era, because nothing keeps that file honest.

**Cost.** The company's API restarts under load, dropping in-flight work, and every restart re-runs
its startup path against a 2.7 GB SQLite database. This is a plausible contributor to the
`database is locked` failures in P48a and to ingest runs that failed for two and a half hours: a
restart mid-import holds and releases write locks unpredictably. It also makes every reading of
"is the API healthy" ambiguous — two systems answer, and they disagree.

**Fix, and do not do it by halves.** Pick ONE owner. systemd already is: delete the cron line (or
reduce it to a *check* that reports and never starts), point `secretary-startup.sh` at
`systemctl restart secretary-api` for the manual case, and retire `~/secretary-api.pid` or make
systemd's `MainPID` the only source. Doing half of it — leaving the cron line but adding a guard —
recreates the fight the first time the guard is wrong.

**Not done in this session**, deliberately: this is the company's live API, it needs a calm window
and a restart under observation, and my mandate tonight was the phone path. Recorded here so it is a
decision someone makes rather than a mystery someone re-discovers.
