...
---
<!-- e:pain|P66|2026-09-14|SECRATARY|open -->
## P66 — The composer doubled the moment a session had usage, and the earlier fix removed the wrong thing

## P67 — the composer's control row doubles into two lines as soon as the session has usage

**What hurts.** The owner's phone composer shows two rows of controls — `+` / attach / cost
pill, then model / context meter / send — and the send control sits beside the field instead
of inside it. He reported it on 2026-09-14 and photographed it still present at 15:06 that
day, *after* the commit that claims to have fixed it (`9eeea6e`, "mobile layer: the composer
fits on one line").

**Why the earlier fix did not hold.** It removed two controls (Access mode, stats strip) to
reduce how often the row overflows. The row itself ships `flex-wrap: wrap`, so overflow is
still allowed and the second group still drops to its own line. Removing controls moved the
threshold; it did not remove the failure. Any session whose row content exceeds the card's
inner width still doubles — and that is every session with a cost pill and a context meter,
i.e. every session that has actually been used.

**Measured, live engine (`phone-gate` :3086 -> engine :3089), 2026-09-14:**

| width | row contents | row height | control bands |
|---|---|---|---|
| 393 | cost pill | 52 | 2 |
...
**Fixed 2026-09-14** in `harness-config/assets/mobile.css` (block "the composer row must not
wrap"): `flex-wrap: nowrap` on the control row, plus `min-width: 0` / `overflow: hidden` on
the two groups so content truncates rather than pushing the send button off the card.
Verified against the served layer with the real markup composed in: row 52 px and 2 bands at
320/358/375/393/430 with every combination, worst-case slack 8 px. See **L214** for the
method, including how the earlier diagnosis was reached and why a one-width test missed it.

**What is still not true.** No real end-to-end turn could be driven headlessly (the composer
input is a `contenteditable` div and a synthetic focus does not take), so the fix is proved on
the composed DOM and the served stylesheet, not on a live streaming turn. The owner confirms
it by reloading on the phone; the layer is `no-store` and read per request, so a reload is
enough and no deploy is involved.

**What would make this class of bug stop recurring.** The phone layer is verified by hand
each time it changes. A one-command check — compose the three session states, resize across
four widths, assert one row band and no overlap — would have caught this in 2026-09-14's
commit and would catch the next regression in the same file.
