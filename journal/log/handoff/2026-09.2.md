...
---
<!-- e:handoff|H77|2026-09-14 15:23 UTC|SECRATARY|open -->
## 2026-09-14 15:23 UTC · SECRATARY · The composer's double layer is fixed at the root: the row was allowed to wrap

## 2026-09-14 15:25 UTC · SECRATARY (engine host) · The composer's two layers are fixed at the root — the row was allowed to wrap

The owner asked whether his screenshot really showed the doubled composer. It did, and the
2026-09-14 commit that claimed to fix it (`9eeea6e`) had not: it removed two controls, which
moved the overflow threshold instead of removing the overflow.

CHANGED

- `harness-config/assets/mobile.css`: new block "the composer row must not wrap".
 `[class*="composer"] [class*="_row"] { flex-wrap: nowrap }`, plus `min-width: 0` and
 `overflow: hidden` on the `_tools` and `_trailing` groups so content truncates instead of
 pushing the send control off the card. Commit **48113bc** here. No deploy, no restart:
 `phone-gate` reads the file per request (`/dsh-phone-mobile.css`, `no-store`).
- Journal: **L214** (method and the measurement table), **P66** (the pain, with what is still
 unproven). NOTE: the P66 body says "P67" once — the tool allocated P66 and the body was
 written before allocation; P66 is the correct id.

MEASURED (live engine, gate :3086 -> engine :3089, 15:0x-15:2x UTC)

Composed the row the way a real session renders it and read every control box over CDP:
row 351x52 with pill only; **104 px and three control bands with pill + context meter**, and
again with the stop control of an active turn. After the change: 52 px and two bands at
320/358/375/393/430 for every one of those compositions, worst-case slack 8 px, no overlap
between the groups. A real render screenshot confirms one row with everything inside the card
and the send button inside the field.

Root cause in one line: the shipped row is `uV2eYG_row{display:flex;flex-wrap:wrap;
justify-content:space-between}`, and the trailing group carries `margin-left:auto`, so when
the row's content exceeds the card width the RIGHT group drops to a second line. The cost pill
(`plugin-cost`, registered in `conversation.input.left`, 20 px tall) and the context meter
(`JObwrW_trigger`, 28x28) are what push it over — which is why an empty session looks perfect
and every used session doubles. A wider viewport is not safer: 430 px doubles with the pill
alone, 393 px does not.

IN FLIGHT

- **The owner must reload once on the phone to see it.** The layer is `no-store` and served per
 request, so a reload is enough. Not yet confirmed on his device.
- Not proved end to end: a headless run cannot start a turn in this app (the composer input is
 a `contenteditable` div, `.value` is inert, and a synthetic focus does not take ownership),
 so this is the composed DOM plus the served stylesheet, not a live streaming turn.

BROKEN / NOT DONE

- The layer still has no automated check. P66 states the fix: a one-command probe that composes
 the three session states, sweeps four widths and asserts one band with no overlap. The probe
 code exists only in `/tmp/mob/` (CDP driver `cdp.py`, composition probe `cdp13.py`) and will
 be lost with that directory — move it into `harness-config/scripts/` before it is.
- `journal.py import-flat`: five flat files still hold entries the log lacks (8 HANDOFF, 4
 LESSONS, 2 PAIN, 10 DECISIONS, 3 WINS). One of them, `HANDOFF.md` "2026-09-14 15:30 UTC ·
 The labelling pilot is built", is from another machine and later than this entry.

NEXT

1. Move the CDP probe into `harness-config/scripts/` and wire it as the phone layer's check.
2. `journal.py import-flat`, then retire the flat files.
3. Owner question #16 is still the top queue row for anything that ships the frontend (Netlify
 credits); the Cloudflare migration is built and verified but needs his token.

EVIDENCE

Probe runs on the authority, 2026-09-14 (all in this session): raw counts in
`/tmp/mob/{compose,c320,c358,c375,c393,c430,v320…v430}.json`; served-CSS check
`curl localhost:3086/dsh-phone-mobile.css` (now 14,530 bytes, contains the new rule); commit
48113bc; the fixed-render screenshot `/tmp/mob/shot.json.png`.

---
<!-- e:handoff|H78|2026-09-14 16:00 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H79|2026-09-14 15:30 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H80|2026-09-14 11:05 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H81|2026-09-14 13:10 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H82|2026-09-14 11:40 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H83|2026-09-14 10:20 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H84|2026-09-14 09:30 UTC|ZABZ-YOGA|open -->
...
---
<!-- e:handoff|H85|2026-09-14|ZABZ-TECH|open -->
...
---
<!-- e:handoff|H86|2026-09-14|ZABZ-TECH|open -->
## 2026-09-14 · ZABZ-TECH · The LPT website-overhaul + Google-Forms→LPT-sign-in arc is now one indexed document
