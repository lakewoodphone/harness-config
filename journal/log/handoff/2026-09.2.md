...
---
<!-- e:handoff|H76|2026-09-14 14:55 UTC|SECRATARY|open -->
## 2026-09-14 14:55 UTC · SECRATARY · The phone path's three deaths were its own shell, the composer is one row, and the workspace has a name

**CHANGED**
- **PAIN P59 RESOLVED, and the cause was not what three earlier readings said.** The gate (and the
 engine, and the redirector) were living inside transient **login-session scopes** because
 `setsid nohup` detaches a terminal but not a cgroup; when the SSH session that started them ended,
 systemd-logind waited for the scope and removed it, killing them silently. Evidence:
 `/proc/<gate>/cgroup -> session-25033.scope` and
 `14:47:06 Session 25028 logged out. Waiting for processes to exit.` → `14:47:16 Removed session
 25028.` **Fixed properly:** `phone-engine.service` + `phone-gate.service` in `/system.slice/`,
 `Restart=always` (2-3 s), `LimitNOFILE=8192` on the gate.
- **One supervisor per process.** `serve-phone.sh` defers to the units (start/stop/status via
 systemctl, never restarting a healthy one). It also keeps the client plugins installed and the
 workspace registered, because those are host state in no git repo.
- **The composer is one row** (mobile.css): the access-mode control and the shipped stats strip are
 hidden on phones, so `Commands · Add attachment · cost pill · model · context · Send` fit on one
 line. Measured after: one control row (y≈470/472 against 678/734), nothing under 44px.
- **The phone workspace is `/home/zabz/phone`** ("Phone"), first in the order, kept by
 `scripts/ensure-phone-workspace.py`; verified from the phone page that a new session lands there.
 `_scratch` is deliberately left registered — nine sessions store it as their cwd and DSH cannot
 open a session whose directory is gone.
- Docs: `docs/dsh-mobile/02-SYSTEMS.md` now names the units, the workspace, and the reason the
 composer changed.

**IN FLIGHT**
- **The company API has two supervisors** (new pain entry, with evidence): the cron watchdog's
 `secretary-startup.sh` kills the process `secretary-api.service` owns and starts its own, so the
 API restarted 4 times in the 14:00 hour and 12 times today. Needs a calm window and one owner
 chosen; deliberately not touched tonight.
- **First run on a brand-new client** still shows a "Choose workspace" row: the app resolves a
 session's workspace by membership, and a session created outside the picker has a directory but no
 membership. His phone does not see it; a fresh device does.

**BROKEN**
- Nothing on the phone path: probe **15/15**, gate and engine systemd-owned. Findings still reach
 nobody except through the in-harness badge (P55 family).

**NEXT**
1. Decide the API's single supervisor and do it under observation (the new pain entry names the
 exact three edits).
2. P46's remainder: seed workspace membership so a first-run client never asks.

**EVIDENCE**
- `cat /proc/<gate>/cgroup` → `session-25033.scope`; journal `Session 25028 logged out … Removed
 session 25028`; after the units: `/system.slice/phone-gate.service`, probe 15/15,
 `serve-phone.sh` → `gate already running: pid 3463650 (systemd)`.
- Composer, live at 393x852: 232px and a wrapped row before; one control row after, access mode
 0x0, `statsShown: false`, `under44: []`.
- Workspace: `session/list` from the phone page → newest session `cwd: /home/zabz/phone`.
- Commits: `1083ca6` (systemd units + defer), `9eeea6e` (composer), `6ca78cb` (workspace + keeper).
